	...
	public String toString() {
		...
		for(int x = 0; x < createMaze.length; x++) {
			for (int y = 0; y < createMaze.length; y++) {
				...
				if((x == 0 && y == 1) || (x == size * 2 && y == (size * 2) - 1)) { // x is 0 and y is 1(start) or x is 8 and y is 7(end)
						createMaze[x][y] = " "; // entry of the maze, or exit of the maze
				}
				else if(createMaze[x][y] == null && x % 2 == 0 && y % 2 == 0) { // if current cell is empty and x,y are even, put a +
